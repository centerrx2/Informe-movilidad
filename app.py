...
def compute_rot_cadera(tree_cadera):
    # ================= DERECHA =================
    R_seg = tree_cadera["sensorData"][3]["orientation"] ##--> right lower leg
    eu_R = quat2eul_matlab(R_seg)

    R_offset = (eu_R[0, 0])          # valor inicial
    R_internal_raw = abs(np.min(eu_R[:, 0]))
    R_external_raw = abs(np.max(eu_R[:, 0]))

    R_internal = abs(R_internal_raw + R_offset)
    R_external = abs(R_external_raw - R_offset)


    # ================= IZQUIERDA =================
    L_seg = tree_cadera["sensorData"][6]["orientation"] ##--> Left lower leg
    eu_L = quat2eul_matlab(L_seg)

    L_offset = (eu_L[0, 0])          # valor inicial
    L_internal_raw = abs(np.max(eu_L[:, 0]))
    L_external_raw = abs(np.min(eu_L[:, 0]))

    L_internal = abs(L_internal_raw - L_offset)
    L_external = abs(L_external_raw + L_offset)
...
def compute_thomas(tree_d, tree_i, start_frame=0, debug_plot=True):
    """
    Thomas (según tus MATLAB):
    - Muslo = max orientación del sensorData en Y (pitch)
        * Derecha -> sensor 10
        * Izquierda -> sensor 13
    - Rodilla = max flexión de rodilla usando jointData (columna 2)
    - debug_plot=True -> muestra gráfica en Streamlit y marca el máximo
    """

    # ===================== DERECHA =====================
    # ✅ MATLAB: sensor 10
    q_R_thigh = tree_d["sensorData"][2]["orientation"]  # [qw qx qy qz]--> Right upperLeg
    eu_R_thigh = quat2eul_matlab(q_R_thigh, order="ZYX")  # [Z Y X]
    signal_R = eu_R_thigh[:, 1]  # ✅ Y = pitch

    # Max desde start_frame
    signal_R_cut = signal_R[start_frame:]
    R_thigh = float(np.max(signal_R_cut))
    idx_R = int(np.argmax(signal_R_cut) + start_frame)

    # RightKnee flexión (jointAngle[:,2])
    knee_R = tree_d["jointData"][JOINT_MAP["RightKnee"]]["jointAngle"][:, 2]
    R_knee = float(np.max(knee_R[start_frame:]))

    # ===================== IZQUIERDA =====================
    # ✅ MATLAB: sensor 13
    q_L_thigh = tree_i["sensorData"][5]["orientation"]##--> Left upper leg
    eu_L_thigh = quat2eul_matlab(q_L_thigh, order="ZYX")
    signal_L = eu_L_thigh[:, 1]  # ✅ Y = pitch

    signal_L_cut = signal_L[start_frame:]
    L_thigh = float(np.max(signal_L_cut))
    idx_L = int(np.argmax(signal_L_cut) + start_frame)

    # LeftKnee flexión
    knee_L = tree_i["jointData"][JOINT_MAP["LeftKnee"]]["jointAngle"][:, 2]
    L_knee = float(np.max(knee_L[start_frame:]))
    ...
    if st.button("Generar informe"):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            if loaded_tests.get("AKE") is not None:
                ...
                results["AKE"] = [
                    ("Mín. extensión rodilla derecha", R_ake),
                    ("Mín. extensión rodilla izquierda", L_ake),
                ]

            # ================= ROTACIÓN CADERA =================
            if loaded_tests.get("ROT_CADERA") is not None:
                R_int, R_ext, L_int, L_ext = compute_rot_cadera(loaded_tests["ROT_CADERA"])
